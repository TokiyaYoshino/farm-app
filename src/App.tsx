...
interface CropAdviceMessage {
  ...
  /** 相談対象の作物。null は作付けに紐づかないスレッド（畑全体・主題だけの相談） */
  crop_id: number | null;
  role: "user" | "assistant";
  content: string;
...
export default function App() {
  ...
  // 開閉は activeThreadId が持つ（null なら閉じている）。adviseCropId は開いている
  // スレッドに紐づく作付けで、照合の対象を決めるためだけに使う（null は作付けなしの主題）
  ...
  const adviseMatches = useMemo(
    ...
    () => (activeThreadId === null ? [] : matchActions(
      adviseActions,
      adviseCropId == null ? reports : reports.filter(r => r.crop_id === adviseCropId),
    )),
    [adviseActions, reports, adviseCropId, activeThreadId],
  );
  ...
  /** 作付けに紐づかない相談。移行SQLが作る受け皿スレッドがあればそれを開き、無ければ立てる。
   *  作物を1件も登録していなくても相談が成立する経路（docs/decisions/20260906-general-advice-entry.md） */
  const openGeneralThread = async () => {
    if (!threadsReady) return showToast("相談はまだ利用できません", "warn");
    setTab("advice");
    const existing = threads.find(t => t.crop_id == null && t.system_key == null);
    if (existing) { void openThread(existing.id); return; }
    await createThread("畑全体の相談", null);
  };
  ...
  const sendAdvise = async () => {
    ...
    try {
      ...
      const workCounts = formatWorkCountsForPrompt(cropReports, 2000);
      const sprayBudget = Math.max(0, AGG_MAX - workCounts.length - 2);
      const aggregates = [
        formatSprayHistoryForPrompt({ reports: cropReports, crops, pesticides, maxChars: sprayBudget }),
        workCounts,
  ...
  const advisePhotoResult = (d: DiagnosisResult, cropId: number | null) => {
    ...
    if (cropId != null) void openThreadForCrop(cropId);
    else void openGeneralThread();
  };
        ...
        <div style={S.page}>
          ...
          <div style={{ display:"flex", gap:8 }}>
            <input
              ...
              placeholder={advisePhoto ? "気になることがあれば書く（空でも送れます）" : "聞きたいことを書く"}
