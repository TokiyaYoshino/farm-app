...
| ai_outputs | id(uuid), organization_id(FK, not null), kind('diagnosis'/'pest_advice'/'daily_report'/'voice_structure'/'advice'/'record_search'), entry_point, report_id(→reports), target_date, field, crop_id(→crops), input_summary, output_json(jsonb), output_text, model, usage(jsonb), cost_usd, created_by(→users), created_at |
...
- `ai_outputs.entry_point` は「どのボタンから呼ばれたか」の識別子（画面_機能）。`kind` とは別軸で、同じ `kind` でも置き場所が複数あるため（`diagnosis` は3か所）、置き場所の効果測定にはこちらを使う。値は `src/App.tsx` の `AiEntryPoint` 型が正。**導線を動かしたら新しい値を足す**（既存値の意味を変えると変更前後の比較が壊れる）。SQLは`scripts/migrations/2026-09-08-ai-outputs-entry-point.sql`
