...
デモデータは `node scripts/seed-demo-reports.mjs <ユーザーID> <パスワード>` で投入できる
（`--delete` で撤収。note が `[demo]` で始まる記録だけを消す）。

- 既定で60件・430日ぶんを入れる（分析画面の「前年同時期比」を出すため1年以上遡る）。
  件数と期間は `--count=` / `--days=` で変えられる
- 記録は南高梅の収穫期（6〜7月）に寄せ、作業種別も月に合わせて選ぶ。気温は月別の平年値ベース
- **作物と圃場は作らない。** 先にアプリか Web で登録しておくこと（マスタまで作ると撤収で消し残る）
- 中身を先に見たいときは `--dry-run`（DBに触らず、ログインも不要）
- 同じ引数なら毎回同じデータになる。スクショを撮り直しても画面が変わらない
