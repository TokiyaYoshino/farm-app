...
相談が見ている作業記録は直近60件・7500字だけなので、そこに無い記録を探す質問には答えようがない。strict スキーマに `record_search_query`（`string | null`）を足し、記録検索（`api/search-chat.ts`）へ渡す検索語を返させる。

**どちらが何に答えられるかは非対称なので、渡す条件をそこに合わせている**（2026-09-08 に実データで確認し、当初のプロンプトの代表例が逆だったのを修正した）。

| 質問 | 相談 | 記録検索（直近180日 or 絞り込み範囲） |
|---|---|---|
| 「去年の防除は何回？」 | ○ 集計が**全年ぶん**の年×作業種別を持つ | × 180日窓の外 |
| 「去年の秋の防除は？」 | △ 年単位までなので月は出せない | × 180日窓の外 |
| 「先月の防除は何回？」 | △ 同上 | ○ |
| 「◯◯を使ったのはいつ？」 | △ 直近60件に入っていれば | ○ |
| 圃場・担当者で絞る | △ 同上 | ○ |

年単位の回数は**渡さずに相談が自分で答える**（集計に載っている）。180日より前は**渡しても答えられない**ので、分かる範囲を答えて分からないと述べさせる。`follow_up_question` と同型にしたのは、**置き場のある方へ行く**ことが実測で分かっているため。

その裏返しで枠を作ると入れたがるので、3重で抑える。

1. プロンプト —— 条件を「記録を数え直す/探し直すことでしか確かめられない質問」に絞り、**渡し先が見る範囲（直近180日 or 絞り込み）を伝えたうえで**null にする例を並べ、「迷ったら null」と明示
