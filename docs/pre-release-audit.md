...
## 1. RLS が `allow_all` — **必須**

**実行準備は完了（2026-09-05）。あとは Supabase ダッシュボードで流すだけ。**

- 当日の手順: **`docs/rls-runbook.md`**（クリックレベル。これだけ見れば進められる）
- 背景・切り戻しの判断: `docs/rls-rollout.md`
- 検証: `scripts/verify-rls.sh`

準備の過程で本体SQLの問題を2つ見つけて修正SQLを追加した（下の 3 と 15）。
**本体をそのまま流すだけでは不十分**なので、必ず runbook の順序に従うこと。
