# 引き継ぎ：記録入力画面の再設計（2026-08-09 時点 / 2026-08-10 追記）

このドキュメント1枚で状況が分かるようにしてある。**結論：モックの作り直しは止めた。実装より先に一次情報を取るべき段階。**

> ## ⚠️ この節は古い（2026-09-17 追記）
>
> 下の「最優先の未着手2つ」は **2026-08-10 時点**の状態で、**どちらも状況が変わっている**。
> 現在地は `docs/decisions/20260912-release-line.md` と `docs/worker-handoff-test.md` を見ること。
>
> | 当時の記述 | 2026-09-17 の実態 |
> |---|---|
> | 越境バグ・RLS 未適用が確定 | **RLS は 2026-09-05 に適用済み。** 09-12 に anon キーで実測したところ残っていた穴は `advice_threads` / `work_categories` の2表で、`scripts/migrations/2026-09-12-rls-anon-leaks.sql` を流せば塞がる（**実行はまだ**） |
> | worker アカウントを1つ作って現場に渡す | **アカウントの作成は 2026-09-16 に完了。** 残りは「渡して1件記録してもらう」だけ（`docs/worker-handoff-test.md`） |
>
> ただし**2.5章の「核心の質問」（作業者アカウントを作ろうとして、どこで止まったか）は生きている**。
> 入力UIより先に配布で止まっている可能性を測る、というこの文書の主旨は変わっていない。
>
> ## 🔴 次のセッションはまずここを読む（2026-08-10 現在の状態）
>
> **最優先の未着手は 2つ**。この順序。どちらも入力UIより先。
>
> | | やること | なぜ | 承認 |
> |---|---|---|---|
> | **0** | **越境バグを塞ぐ**（2.6章の手順1〜6） | anon key だけで `users` 5件・`reports` 33件などが素で読める。RLS 未適用が確定（`jwt_organization_id()` が `PGRST202`）。**公開前必須** | 手順2（id 振り直し）が**本番データ更新なので要承認・未取得** |
> | **1** | **worker アカウントを1つ作って現場に渡す** | worker が0人＝**入力UIは一度も現場で試されていない**。ここを埋めないと入力UIの議論は全部机上。作成は Web 版のユーザーピッカー→「管理画面」から | — |
>
> **RLS を先に流しても分離されない**（全員の `organization_id` が同一値なので JWT クレームも同一）。
> 必ず 0. のデータ振り直しが先。→ 10章1. と 2.6章
>
> ### 完了したもの（2026-08-10）
>
> - **11章 入力途中の消失バグ = 修正済み。ただし未コミット**（テストは 21 passed）。実機確認待ち
> - **12章 農業エージェント = 本番稼働**（`main` の `1e37192`）。マイグレーション2本適用済み・デプロイ済み。
>   実測で `work_type` が3件すべて作業記録の語彙に一致（設計時の最大の懸念が解消）。
>   **残は FAMIC 紐付け（要承認・未実行）と実機確認だけ**
>
> ### 引き継ぎで踏みやすい罠（実際に踏んだもの）
>
> - **`organizations` が anon で0件に見えるのは行が無いからではない**（RLS でポリシーが無いため）。
>   実際には kishu の行が元からあった。→ 2.6章の訂正
> - **`/api/advise` の 405 は未デプロイの根拠にならない**（`vercel.json` の catch-all rewrite）。
>   本文を見る。正常なら引数無しの POST に `{"error":"crop.name required"}`
> - **Supabase SQL Editor は複数文を流すと結果を表示しない**（「成功。返された行はありません」だけ出る）。
>   確認したいときは 1文の select にする（`2026-08-10-organizations-verify.sql`）
...
>
> **→ 2026-08-10 に「入力途中の消失」は修正済み（下記11章・未コミット）。
> 「次にやる作業が分からない」＝農業エージェントも本番稼働（12章）。**

> **2026-08-10 追記：この優先順は変わっていない。** 農業エージェントを作ったのは
> 競合レビューで出た独立項目（並行着手できるもの）を消化したためで、0.（越境バグ）と
> 1.（worker アカウント）を飛ばしたわけではない。**この2つが最優先のまま残っている。**
> なお 12章の作業で `organizations` に kishu の行が実在することが確定したので、
> 0. の手順1は「行を作る」ではなく「asuka の行を足す＋id を振り直す」になった（2.6章の訂正）。
...
## 11. 入力途中の消失バグ（2026-08-10・**修正済み・未コミット**）

> **⚠️ この章の変更は git に入っていない（2026-08-10 時点）。** 12章（農業エージェント）とは
> 別件なのでコミットを分けたまま作業ツリーに残してある。`git status` で以下が出る:
>
> ```
>  M expo-prototype/screens/QuickReportSheet.tsx
> ?? expo-prototype/lib/reportDraft.ts
> ?? expo-prototype/scripts/test-report-draft.mjs
> ?? expo-prototype/scripts/analyze-input-usage.mjs   # 2章の実測スクリプト（読み取り専用）
> ?? scripts/migrations/2026-08-10-organizations-verify.sql  # 12章の確認用（1文の select）
> ```
>
> **テストは通っている**（`node scripts/test-report-draft.mjs` → 21 passed / `tsc` clean）。
> 実機確認が済んでいないためコミットを保留しているだけで、壊れているわけではない。
...
>
> **2026-08-10 に本番で稼働開始した。** 残っているのは FAMIC 紐付け（層2）と実機確認だけ。
>
> | # | やること | 状態 |
> |---|---|---|
> | 1 | `2026-08-10-organizations-check.sql` | ✅ 実行済み。`organizations` に kishu の行が**元からあった**（`orgs_total=1` / 補填0行 / 孤児0） |
> | 2 | `2026-08-10-crop-advisor.sql` | ✅ 適用済み。2テーブルとも PostgREST から 200（旧 404）、`organizations` / `crops` への埋め込みも解決 |
> | 3 | `api/advise.ts` のデプロイ | ✅ `main` にマージ・push 済み（`d446677`）。**本番で 200 応答を確認** |
> | 4 | `link-famic-crop-names.mjs --apply` | ❌ **未実行**。7件すべて `null` のまま。層2（農薬）が常に空になる。**本番データ更新なので承認が必要** |
> | 5 | 実機確認（スレッドの読み込み・保存・「これはやらない」） | ❌ 未実施 |
>
> **本番での実測（2026-08-10）**: `crop=キャベツ` / `start_date=2026-06-20` で POST → 200。
> やること3件が返り、**`work_type` は3件すべて語彙に完全一致（`施肥` / `防除` / `除草`）＝ `unmatchable` ゼロ。**
> 最大の未知だった「語彙にどれだけ当たるか」は、少なくとも一般的な作業名では当たる。1回 ¥0.07（$0.00044）。
> `registrationFacts` は 0 件で、`limits` に「作物名が紐付いていないため薬剤の使用可否は判断していません」が
> 正しく出た（4 が未実行なので**設計どおりの縮退**）。
>
> **`/api/advise` が 405 を返すのは「未デプロイ」の根拠にならない。** `vercel.json` の catch-all
> rewrite により存在しないパスも 405 になる。稼働確認は POST してレスポンス本文を見る
> （デプロイ後は引数無しの POST に `{"error":"crop.name required"}` が返る＝ハンドラ自身が応答している）。
...
品種名や表記ゆれの誤判定が使用者を法令違反に導くため）。下表は**推測ではなく、
...
7件すべて品種名を含まない一般名なので、品種名による曖昧さは無い。FAMIC が半角カナで持つ
