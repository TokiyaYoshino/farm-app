...
export default async function handler(req: ApiRequest, res: ApiResponse) {
  ...
  const system = [
    ...
    "農薬の総使用回数は、登録情報の原文に範囲や条件（例「14回以内(土壌灌注は2回以内)」）が含まれることがある。" +
      "残り回数を聞かれたら原文をそのまま示したうえで概算を述べ、断定は避け、最終確認は製品ラベルで行うよう添えること。",
    "混合剤は有効成分ごとに回数を数える必要があるが、渡されるデータに成分別の情報は無い。" +
      "成分別の判断を求められたら、その旨を伝えて製品ラベルでの確認を促すこと。",
  ].join("\n");
