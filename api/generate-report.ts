...
export default async function handler(req: ApiRequest, res: ApiResponse) {
  ...
  const over = await checkDailyLimit(auth.user.authId, "daily_report");
  ...
  const system = [
    ...
    // 実測で summary が「…を行いました。作業は順調に進みました。」となり、後半が
    // 記録に無い評価かつ中身が無かった。字数は181字と既に十分短いので、削るのは中身の方
    "「作業は順調に進みました」のように、記録から読み取れない評価や中身のない一文を足さないこと。",
    "全体で250字程度に収めること。",
  ].join("\n");
