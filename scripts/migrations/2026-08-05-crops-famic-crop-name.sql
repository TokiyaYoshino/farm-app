...
-- crops.name（農家が付けた名前・例「ほうれん草」）と pesticide_registrations.crop_name
-- （FAMIC の登録上の作物名・例「うめ」）を対応させる必要がある。
-- 文字列の自動マッチングは「ほうれん草」≠「ほうれんそう」のように失敗し、誤判定は使用者を
...
comment on column crops.famic_crop_name is
  'FAMIC 登録適用部の作物名（例: ほうれん草 → ほうれんそう）。使用回数の突き合わせに使う紐付け。未設定は判定不可として扱う';
